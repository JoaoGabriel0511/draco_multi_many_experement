function run_draco_many {
    ...
    for idx in "${!GRAPH_FILES[@]}"; do
        ...
        /usr/bin/time -v -o ${LOG_PATH}/${EXPERIMENT_NAME}/${mdg_name}.out ${BASE_PATH}/main --many < ${GRAPHS_PATH}/${GRAPH_FILES[$idx]} > ${EXP_PATH}/${EXPERIMENT_NAME}/${mdg_name}.dot
    done
}
...
function run_script {
    ...
    if [ "$1" = "" ]; then
        ...
        if [ "$EXPERIMENT_NAME" = "" ]; then
            ...
            run_draco_many $EXPERIMENT_NAME
        fi
    fi
}
