...
public class SMPSO extends Algorithm
{
	...
	protected double trueHypervolume_;
	...
	private void computeSpeed(int iter, int miter) throws JMException, IOException
	{
		...
		for (int i = 0; i < particlesSize_; i++)
		{
			...
			for (int var = 0; var < particle.getNumberOfDecisionVariables(); var++)
			{
				...
				speed_[i][var] = velocityConstriction(constrictionCoefficient(C1, C2) * (inertiaWeight(iter, miter, wmax, wmin) * speed_[i][var] + C1 * r1 * (bestParticle.getValue(var) - particle.getValue(var)) + C2 * r2 * (bestGlobal.getValue(var) - particle.getValue(var))), deltaMax_, // [var],
						deltaMin_, // [var],
						var, i);
			}
		}
