...
public class XReal
{
	...
	public XReal()
	{
		try
		{
			...
			ARRAY_REAL_AND_BINARY_SOLUTION = Class.forName("jmetal.base.solutionType.ArrayRealAndBinarySolutionType");
		}
	...
	{
		if ((type_.getClass() == REAL_SOLUTION) || (type_.getClass() == BINARY_REAL_SOLUTION))
		{
		...
		else if (type_.getClass().equals(ARRAY_REAL_AND_BINARY_SOLUTION))
		{
			return ((ArrayReal) (solution_.getDecisionVariables()[0])).array_[index];
		}
		else
	...
	public double getLowerBound(int index) throws JMException
	{
		...
		{
			Configuration.logger_.severe("jmetal.util.wrapper.XReal.getLowerBound, solution type " + type_ + "+ invalid");
	...
	public double getUpperBound(int index) throws JMException
	{
		if ((type_.getClass() == REAL_SOLUTION) || (type_.getClass() == BINARY_REAL_SOLUTION))
			...
			Configuration.logger_.severe("jmetal.util.wrapper.XReal.getUpperBound, solution type " + type_ + "+ invalid");
	...
	public int size()
	{
		if ((type_.getClass().equals(REAL_SOLUTION)) || (type_.getClass().equals(BINARY_REAL_SOLUTION)))
			return solution_.getDecisionVariables().length;
		else if (type_.getClass().equals(ARRAY_REAL_SOLUTION))
			return ((ArrayReal) (solution_.getDecisionVariables()[0])).getLength();
		else
			Configuration.logger_.severe("jmetal.util.wrapper.XReal.size, solution type " + type_ + "+ invalid");
		return 0;
