...
public class Solution implements Serializable
{
	...
	public int getNumberOfBits()
	{
		...
		for (int i = 0; i < variable_.length; i++)
			try
			{
				if ((variable_[i].getVariableType() == Class.forName("jmetal.base.variable.Binary")) || (variable_[i].getVariableType() == Class.forName("jmetal.base.variable.BinaryReal")))
